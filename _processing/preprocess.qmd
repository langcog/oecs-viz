...
pubs_tf <- prepped_pubs |>
  group_by(article) |>
  mutate(tf = count / sum(count)) |>
  ungroup()

pubs_idf <- prepped_pubs |>
  group_by(lemma) |>
  summarise(n_articles = n()) |>
  mutate(idf = log(sum(n_articles) / n_articles))

pubs_tfidf <- pubs_tf |>
  left_join(pubs_idf) |>
  mutate(tfidf = tf * idf) |>
  select(article, lemma, tfidf)
...
pubs_glove <- pub_lemmas_glove |>
  group_by(article) |>
  summarise(across(starts_with("V"), \(v) weighted.mean(v, tfidf)))
