...
# get pub slugs
pub_slugs <- map_chr(pubs, \(pub) pub |> pluck("slug")) |>
   enframe(name = "name", value = "slug")
...
pub_themes <- themes |> enframe(name = "theme", value = "name") |> unnest(name) |>
  group_by(name) |> summarise(themes = list(theme)) # |> deframe()
  # relocate(pub, .before = everything()) |> deframe()
# nest(themes = theme) |> 
# transpose(themes)

pub_meta <- pub_slugs |> left_join(pub_themes)
write_rds(pub_meta, "data/pub_meta.rds")
```
