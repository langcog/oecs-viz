...
pub_graph <- read_rds(here("data/pub_graph_full.rds"))
...
links <- pub_graph |>
  as_data_frame("edges") |>
  as_tibble() |>
  select(source = from, target = to, value = similarity_norm) |>
...
viewof k = Inputs.range([1, 8], {
  label: "Include closest k neighbors for each article",
  step: 1,
  value: 2
})

viewof s = Inputs.range([0.9, 1], {
  label: "Include links with similarity at least",
  step: 0.01,
  value: 0.95
})
```
